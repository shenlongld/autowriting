...
- `generated/assets/slide_*.png`：每页左侧嵌入的高分辨率本地图表/架构图。

## 中间代码

- `build_vllm_sosp2023_ppt.py`：使用 `Pillow` 本地绘制 8 张高分辨率图表，再使用 `python-pptx` 生成 8 页 PPT。
- `requirements-vllm-ppt.txt`：复现生成脚本所需依赖。

## 版式说明

- 每页统一采用“左侧图表 / 右侧说明”的结构。
- 左侧图表为本地生成并嵌入的 PNG，避免使用粗糙的 PPT shape 拼图。
- 右侧说明包含：页面主题、核心结论、讲解要点、数据口径说明，适配约 6 分钟学术演示。
...
- 序列长度-时延折线图为趋势示意，用于说明论文结论：长序列、大模型、复杂解码下 vLLM 优势更明显。
